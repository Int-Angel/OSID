...
public class BluetoothConfigurarActivity extends AppCompatActivity {
    //1)
    ...
    public void onResume() {
        ...
        if (devices.size() > 0) {
            for (BluetoothDevice devices1 : devices) { //EN CASO DE ERROR LEER LA ANTERIOR EXPLICACION
                mPairedDevicesArrayAdapter.add(devices1.getName() + "\n" + devices1.getAddress());
            }
        }
    }

    // Configura un (on-click) para cada dispositivo que aparezca en la ListvView
